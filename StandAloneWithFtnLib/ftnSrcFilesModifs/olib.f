...
c     initialize system props/flags
      ...
      !write(*,*)"ftn:: getloc: bef. insysp"
      call insysp (ssick,ppois,bulkg,bsick)
      !write(*,*)"ftn:: getloc: aft. insysp"
...
c     x3 and convert to y array
               !write(*,*)"ftn:: getloc: bef. xtoy"
               call xtoy (ids,i,.false.,bad)
               !write(*,*)"ftn:: getloc: aft. xtoy"
...
c     get and sum phase properties
         !write(*,*)"ftn:: getloc: bef. getphp"
         call getphp (ids,i,sick,ssick,ppois,bulkg,bsick)
         !write(*,*)"ftn:: getloc: aft. getphp"
         
      end do 
c     compute aggregate properties:
      
      !write(*,*)"ftn:: getloc: bef. gtsysp"
      call gtsysp (sick,ssick,bulkg,bsick)
      !write(*,*)"ftn:: getloc: aft. gtsysp"

99    if (lopt(14)) p = dlog10(p)

      !write(*,*)"ftn:: getloc end"
      !write(*,*)"ftn:: getloc stop!"
      !stop
      end
...
c-----------------------------------------------------------------------
      ...
      if (ids.le.0) then 
         ...
         !write(*,*)"ftn:: moduli: bef shearm 1"
         call shearm (mu,mut,mup,ks,kst,ksp,-ids,ok)
         !write(*,*)"ftn:: moduli: aft shearm 1"
         
      else 

         !write(*,*)"ftn:: moduli: bef if (smod(ids)) then"
         if (smod(ids)) then
            !write(*,*)"ftn:: moduli: in if (smod(ids)) block"
...
c     partial molar volumes for volume fractions
               !write(*,*)"ftn:: moduli: bef. endvol, i=",i
               v0(i) = endvol(jend(ids,2+i), ok)
               !write(*,*)"ftn:: moduli: aft. endvol,i,ok=",i,ok
            ...
            do i = 1, lstot(ids)
               ...
               !write(*,*)"ftn:: moduli: bef shearm 2"
               call shearm (pmu,pmut,pmup,
     *                      pks,pkst,pksp,jend(ids,2+i),ok)
               !write(*,*)"ftn:: moduli: aft shearm 2"
...
c     save the speciation.
      !write(*,*)"ftn:: getphp: bef. getspc"
      if (id.gt.0) call getspc (id,jd)
      !write(*,*)"ftn:: getphp: aft. getspc"
...
c     to refine difference increments
         !write(*,*)"ftn:: getphp: bef. getdpt 2"
         call getdpt (g0,dp0,dp1,dp2,dt0,dt1,dt2,v,gpp,s,gtt,gpt,id,fow,
     *        rxn)
         !write(*,*)"ftn:: getphp: aft. getdpt 2"
         !write(*,*)"ftn:: getphp: stop!"
         !stop
c                                 enthalpy
         e = g0 + t * s
